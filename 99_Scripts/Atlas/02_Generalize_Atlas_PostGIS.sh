...
function drop_Scheme () {

	local schema=${1}

	exe_PSQL "DROP SCHEMA ${schema} CASCADE"
}
...
function get_Table_Names {
	...
	# Create new table
		SQL_Query="SELECT table_name
			...
			"
	 # Execute Query
		export table_Names=$(exe_PSQL "${SQL_Query}")
		# remove empty space
		export table_List=`echo "${table_Names}" | sed -e 's/^[ \t]*//'`
...
function generalize {
	...
	local out_Table="${DB_Schema3}.${in_Name}_gl"
	...
	# Create new table
	 ...
	 echo -e "\nTABLE: \t\t\t ${in_Table}"
		...
		echo -e "\nQUERY 3 - create 2nd topology structure -> \t\t\t\tStarting time:  $(date +"%T") \n\n"
		exe_PSQL "${SQL_Query3}"

		echo -e "\nQUERY 4 - Retrieves polygons -> \t\t\t\t\tStarting time:  $(date +"%T") \n\n"
		exe_PSQL "${SQL_Query4}"
...
export_Parameters
# create_Scheme ${DB_Schema4}

get_Table_Names 2 0
IFS=$'\n'
for names in ${table_List}
do
 echo -e "${names}"
